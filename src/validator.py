...
def validate_dossier(dossier: Dossier) -> Dict[str, Any]:
    ...
    for doc_type in required_docs:
        if doc_type == "devis":
            if not any("devis" in n for n in lower_names):
                uploaded_missing_docs.append(
                    "Document 'devis' non détecté dans les PDF déposés (nom de fichier sans 'devis')."
                )
        elif doc_type == "facture":
            if not any("facture" in n or "fac_" in n or "fs_" in n for n in lower_names):
                uploaded_missing_docs.append(
            ...
            if not any(
                "attest" in n
                or "aft" in n
                or "attestation sur l'honneur" in n
            ...
            if not any("fin de travaux" in n or "aft" in n for n in lower_names):
                uploaded_missing_docs.append(
